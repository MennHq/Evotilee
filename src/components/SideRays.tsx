...
void main() {
  ...
  gl_FragColor = color;
      ...
      const updateSize = () => {
        if (!containerRef.current || !renderer || isDisposed) return;
        ...
        const loop = (t: number) => {
          ...
          if (rendererRef.current && uniformsRef.current && meshRef.current) {
            ...
            try {
              ...
              // Ignore render frame interruptions safely
            }
          }
      ...
      // Setup intersection observer to pause rendering offscreen without destroying context
      ...
      console.warn('WebGL initialization for SideRays failed, falling back to CSS glow:', err);
  ...
  if (hasWebGLError) {
    return (
      <div 
        className={`w-full h-full bg-[radial-gradient(ellipse_80%_80%_at_70%_0%,rgba(255,255,255,0.06),rgba(0,0,0,0))] ${className}`.trim()} 
      />
    );
  }
